...
/** Opt-in local verification. The copyrighted input is never a repository fixture. */
@EnabledIfEnvironmentVariable(named = "NOVEL_TEST_FILE", matches = ".+")
class LocalNovelTest {
    @Test void parsesTheProvidedLocalNovelWithoutChangingItsBytes() throws Exception {
        Path file = Path.of(System.getenv("NOVEL_TEST_FILE"));
        byte[] bytes = Files.readAllBytes(file);
        String before = HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(bytes));
        ...
        for (int i = 0; i < 1507; i++) {
            var number = java.util.regex.Pattern.compile("^第([0-9]+)章").matcher(parsed.chapters().get(i).title());
        ...
        System.out.printf("Local novel: bytes=%d, encoding=%s, chapters=%d, volumes=%d, bodyCharacters=%d, elapsedMs=%d, sha256=%s%n", bytes.length, parsed.encoding(), parsed.chapters().size(), parsed.volumes().size(), parsed.characterCount(), (System.nanoTime() - started) / 1_000_000, before);
    }
}
