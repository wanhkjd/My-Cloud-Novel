...
class RedisSessionIT extends DatabaseIntegrationTest {
    ...
    void csrfLoginFixationProtectionCookieTtlAndLogoutUseRedis() throws Exception {
        ...
        assertThat(String.join(";", login.headers().allValues("set-cookie")))
                .contains(SessionConfig.COOKIE_NAME + "=", "HttpOnly", "SameSite=Lax", "Path=/");
        ...
        assertThat(post("/api/auth/logout", "", logoutCsrf).statusCode()).isEqualTo(204);
    ...
    private String cookieValue() {
        return cookies.getCookieStore().getCookies().stream()
                .filter(cookie -> cookie.getName().equals(SessionConfig.COOKIE_NAME))
    ...
    private String sessionCookie() {
        return SessionConfig.COOKIE_NAME + "=" + cookieValue();
    }
