...
describe('real MySQL / Redis E2E isolation', () => {
  it('never inherits runtime database, Redis database or private original paths', () => {
    ...
    expect(config.SPRING_DATA_REDIS_DATABASE).toBe('14');
    expect(config.SPRING_SESSION_REDIS_NAMESPACE).toMatch(/^cloud-novel:e2e:/);
  ...
  it('rejects host injection and invalid test ports', () => {
    for (const input of [
      ...
      { TEST_MYSQL_PORT: '0' },
      { TEST_REDIS_PORT: '65536' },
      { TEST_REDIS_HOST: 'redis://production' },
    ]) {
  ...
  it('uses a fresh Redis namespace on every run', () => {
    expect(e2eEnvironment({ E2E_DB_PASSWORD: 'synthetic' }).REDIS_NAMESPACE).not.toBe(
      e2eEnvironment({ E2E_DB_PASSWORD: 'synthetic' }).REDIS_NAMESPACE,
    );
  });
});
